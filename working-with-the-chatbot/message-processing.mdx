...
When the ChatAgent receives a message to process, it goes through several stages:

```mermaid
flowchart TD
    Input[Message Input] --> Prompt[Build System Prompt]
    Prompt --> Context[Add Customer Context]
    Context --> History[Include Conversation History]
    History --> Tools[Attach Available Tools]
    Tools --> LLM[Send to Language Model]
    LLM --> Response[Receive Raw Response]
    Response --> Parse[Parse XML Tags]
    Parse --> Extract[Extract Customer Message]
    Extract --> Send[Send to Customer]
    Parse --> Internal[Process Internal Tags]
    Internal --> Actions[Execute Required Actions]
```
...
If the LLM requested tool calls during processing, they're executed between the prompt and final response:

```xml
<thinking>
Customer wants to order "the usual" but I don't know what that is. I need to check their order history.
</thinking>

<!-- Tool call: get_customer_previous_orders -->
<!-- Tool result: Last 3 orders all contained Large Pepperoni pizza -->

<response_for_customer>
I see you love our Large Pepperoni pizza - that's been your go-to! Should I add one to your order for €14.00?
</response_for_customer>
```
