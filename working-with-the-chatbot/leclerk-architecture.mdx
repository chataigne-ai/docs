...
- `LeClerk/src/chataigne_clerk/bot/tool_definitions.py`
  - Defines the tool catalog exposed to the LLM (address checks, menu lookup, update order draft, feedback, scheduling, etc.). For a complete, in-depth reference of every tool, see [LeClerk Tools Reference](/working-with-the-chatbot/leclerk-tools).
