...
{
  id: string,
  internalId: string,  // Id unique à l'échelle du catalogue dans un format lisible pour les LLMs
  ref?: string,        // Référence dans logiciel de caisse
  name: string,
  image?: ImageEntity,
...
```typescript
{
  ...
  image?: ImageEntity,
  available: boolean,
  restriction?: Restriction  // Disponibilité horaire/géographique/type de service
}
```
...
<Note>
  **Particularité de notre vertical restauration** : Contrairement au e-commerce
  classique où un produit peut avoir plusieurs SKUs (ex: T-shirt en différentes
  tailles), dans la restauration, nous maintenons une relation 1:1 entre produit
  et SKU. Cela simplifie la gestion tout en gardant la flexibilité pour évoluer
  si nécessaire.
</Note>
...
- **Structure :**
  ```typescript
  {
    skuId: string,
    sku: Sku,
    price?: Price         // Premium dans le cas où ce SKU est choisi. Exemple un deal menu midi a un prix de base a 10 euros mais choisir un risotto comme plat augmente le prix du menu de 2 euros.
  }
  ```
...
Le système gère des restrictions complexes adaptées à la restauration. Pour l'instant les restrictions ne s'applique qu'aux produits mais nous allons les étendres aux options et aux menus :
...
- Plats disponibles uniquement en livraison

## Intégration avec les systèmes de caisse

### Importance du champ `ref`

Le champ `ref` présent sur les SKUs, Options et Deals est **critique** pour la synchronisation avec les logiciels de caisse. C'est l'identifiant utilisé pour identifier les entités lors de l'envoie des commandes au système de caisse
