...
<AccordionGroup>
  <Accordion title="Logger Personnalisé">
    ```typescript
    @Injectable()
  ...
  <Accordion title="Middleware de Contexte">
    ```typescript
    @Injectable()
...
<AccordionGroup>
  <Accordion title="Par Request ID">
    ```sql
    SELECT * FROM logs
...
<Accordion title="Par Utilisateur">
  ```sql SELECT * FROM logs WHERE userId = 'user-456' ORDER BY timestamp DESC
  LIMIT 100; ```
</Accordion>

  <Accordion title="Analyse d'Erreurs">
    ```sql
    SELECT
...
<AccordionGroup>
  <Accordion title="Dashboards Recommandés">
